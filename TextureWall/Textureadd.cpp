...
double Textureadd::OPattern6(int pointid, int pointnumber, double amplitude) {//map_leaf
	return amplitude*(sin(2 * pointid*1.0 / pointnumber * 2 * PI) + cos(2 * pointid*1.0 / pointnumber * 2 * PI)) - (sin(pointid*1.0 / pointnumber * 2 * PI) + cos(pointid*1.0 / pointnumber * 2 * PI));
}
double Textureadd::OPattern7(int pointid, int pointnumber, double amplitude) {//circle
	if (pointid <= (pointnumber *0.5)) {
		return amplitude*sqrt(0.25*pointnumber*pointnumber - (0.5*pointnumber - pointid)*(0.5*pointnumber - pointid));
	}
	return amplitude*sqrt(0.25*pointnumber*pointnumber - (0.5*pointnumber - (pointnumber-pointid))*(0.5*pointnumber - (pointnumber - pointid)));
}
...
double Textureadd::TPattern3(int pointid, double pointnumber, int layerid, double layernumber, double amplitude) {//金字塔
	double K1 = layernumber * 1.0 / pointnumber;
	double K2 = -layernumber * 1.0 / pointnumber;
